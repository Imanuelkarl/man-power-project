...
export const DashboardPage: React.FC = () => {
  ...
  const avgCapacity = questionnaires.length
    ? (
        questionnaires.reduce((s, q) => s + q.capacityUtilization, 0) /
        questionnaires.length
      ).toFixed(1)
    : "—";
  const totalLoad = questionnaires.reduce(
    (s, q) => s + q.totalEnergyConsumed,
    0,
  );
  const totalEnergy = questionnaires.reduce(
    (s, q) =>
      s + q.energyDiesel + q.energyGas + q.energyGenerator + q.energyOther,
    0,
  );

  const recent = [...questionnaires]
  ...
  return (
    <div className="p-6 lg:p-10 space-y-6 max-w-[1400px]">
        ...
        actions={
          <div className="flex flex-wrap gap-2">
            <Button
              variant="outline"
              size="sm"
              onClick={() => exportCSV(manufacturers, questionnaires)}
            >
            ...
            <Button
              variant="outline"
              size="sm"
              onClick={() => exportExcel(manufacturers, questionnaires)}
            >
              <FileSpreadsheet className="w-4 h-4 mr-2" /> Excel
            </Button>
            <Button
              size="sm"
              onClick={() => exportPDF(manufacturers, questionnaires)}
            >
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Metric
          icon={Zap}
          label="Alt. energy spend"
          value={formatNaira(totalEnergy)}
          accent="energy"
        />
      </div>
